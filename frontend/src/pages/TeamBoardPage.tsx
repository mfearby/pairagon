...
import {
  Stack, Title, Group, Button, TextInput, Anchor, Text, ActionIcon, Badge, MultiSelect
} from '@mantine/core'
import { IconArrowLeft, IconSettings, IconArrowsShuffle, IconTrash, IconCheck, IconGripVertical } from '@tabler/icons-react'
...
function SortableTaskCard({
  ...
  return (
    <div ref={setNodeRef} style={{ ...style, background: 'var(--mantine-color-body)', border: '1px solid var(--mantine-color-default-border)', borderRadius: 8, padding: '12px 16px' }}>
...
export default function TeamBoardPage() {
  ...
  useEffect(() => {
    api.getTeam(id).then(data => {
      setTeam(data)
      setTasks(data.tasks)
    }).catch(() => notifications.show({ message: 'Failed to load team', color: 'red' }))
  }, [id])
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setTasks(reordered)
    try {
      await api.reorderTasks(id, reordered.map(t => t.id))
    } catch {
      notifications.show({ message: 'Failed to save order', color: 'red' })
    }
  }
    ...
    <Stack maw={700} mx="auto">
      <Group justify="space-between">
        <Anchor component={Link} to="/" size="sm">
          <Group gap={4}><IconArrowLeft size={14} /> All teams</Group>
        </Anchor>
        <Anchor component={Link} to={`/teams/${id}/settings`} size="sm">
          <Group gap={4}><IconSettings size={14} /> Settings</Group>
        </Anchor>
      </Group>

      <Group justify="space-between">
        <Title order={2}>{team.name}</Title>
        <Button leftSection={<IconArrowsShuffle size={16} />} variant="light" onClick={handleRandomize}>
          Randomize pairs
        </Button>
      </Group>
      ...
      {tasks.length === 0 && (
        <Text c="dimmed" ta="center" mt="xl">No open tasks. Add one above.</Text>
      )}
    </Stack>
  )
}
